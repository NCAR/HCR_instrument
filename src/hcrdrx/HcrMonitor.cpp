...
float
HcrMonitor::procEnclosureTemp() const {
...
float
HcrMonitor::txEnclosureTemp() const {
    QMutexLocker locker(&_mutex);
    return _dequeAverage(_txEnclosureTemps);
}

float
HcrMonitor::rxTopTemp() const {
    QMutexLocker locker(&_mutex);
    return _dequeAverage(_rxTopTemps);
}

float
HcrMonitor::rxBackTemp() const {
    QMutexLocker locker(&_mutex);
    return _dequeAverage(_rxBackTemps);
}

float
HcrMonitor::rxFrontTemp() const {
    QMutexLocker locker(&_mutex);
    return _dequeAverage(_rxFrontTemps);
}

float
HcrMonitor::hTxPowerVideo() const {
    QMutexLocker locker(&_mutex);
    return _hTxPowerVideo;
}

float
HcrMonitor::vTxPowerVideo() const {
    QMutexLocker locker(&_mutex);
    return _vTxPowerVideo;
}

float
HcrMonitor::testTargetPowerVideo() const {
    QMutexLocker locker(&_mutex);
...
bool
HcrMonitor::wgPressureGood() const {
    QMutexLocker locker(&_mutex);
    return _wgPressureGood;
}

bool
HcrMonitor::locked100MHz() const {
    QMutexLocker locker(&_mutex);
    return _locked100MHz;
}

bool
HcrMonitor::gpsTimeServerGood() const {
    QMutexLocker locker(&_mutex);
...
double
HcrMonitor::_lookupQEAPower(double voltage) {
