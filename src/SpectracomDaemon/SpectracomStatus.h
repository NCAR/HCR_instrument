...
public:
    ...
    /// @brief Return true iff replies are being received from the Spectracom.
    /// @return true iff replies are being received from the Spectracom.
    bool deviceResponding() const { return(_deviceResponding); }
    
    /// @brief Return true iff the Spectracom's 1 PPS output is currently
    /// locked to 1 PPS from GPS.
    /// @return true iff the Spectracom's 1 PPS output is currently
    /// locked to 1 PPS from GPS.
    ...
    /// @brief Return the time difference between Spectracom 1 PPS and
    /// GPS 1PPS.
    /// @return the time difference between Spectracom 1 PPS and GPS 1PPS.
...
private:
    ...
    '_statusTime', '_hostName', '_hostResponding', '_reference', 
    ...
    /// Is the Spectracom responding on the serial port?
    ...
    /// Is the Spectracom currently in holdover?
    bool _inHoldover;

    /// Frequency error estimate
    float _freqErrorEstimate;

    /// Time difference between Spectracom output 1 PPS and GPS 1 PPS, in
    /// seconds.
    float _timeDiff1PPS;

    /// Health status value from the Spectracom
    uint16_t _healthStatus;

    /// Has an error in the Spectracom's configuration been detected?
    bool _configError;
};
