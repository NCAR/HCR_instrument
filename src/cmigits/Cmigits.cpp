...
Cmigits::getStatus() {
    ...
    if (_simulate) {
        return(CmigitsStatus::simulatedStatus(_simFilamentOn, _simHvOn));
    }
    
    // This must be the real thing, so get status from the C-MIGITS
    CmigitsStatus status;

	// Reply from the C-MIGITS, which will be parsed to build the Status
	// struct.
	static const int REPLYSIZE = 20;
	uint8_t reply[REPLYSIZE];

	// Max number of times to attempt to get good status from the C-MIGITS
    ...
    for (attempt = 0; attempt < MAX_ATTEMPTS; attempt++) {
    	...
    	try {
    	    return(CmigitsStatus(reply));
    	} catch(CmigitsStatus::ConstructError & e) {
            // Report the error.
    	    ELOG << e.what();
...
void
Cmigits::_sendCommand(uint8_t desiredState) {
    if (_simulate)
        return;
